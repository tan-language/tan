use tan::{ann::ANNO, eval::env::Env, expr::Expr};
...
fn env_binds_names_to_values() {
    ...
    assert!(matches!(env.get("a"), Some(ANNO(Expr::Symbol(sym), ..)) if sym == "hello"));
...
fn env_bindings_can_be_updated() {
    ...
    assert!(matches!(env.get("a"), Some(ANNO(Expr::Symbol(sym), ..)) if sym == "world"));
}
